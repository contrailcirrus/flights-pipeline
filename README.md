...
## Primer
See the [Impact Explorer](https://explore.contrails.org) and the [Inventory REST API](https://api.contrails.org/openapi#/Inventory) endpoints 
for more context on the dataset generated by this pipeline.

### Contrail Impact Estimation & Assumptions
- Contrail impact is estimated using the [pycontrails](https://py.contrails.org/) CoCiP trajectory model, applied to flight trajectories as observed in ADS-B, using ECMWF ERA5 reanalysis meteorological data.
- The pycontrails CoCiP model is run on flight trajectories resampled to 1-minute frequency, with great-circle interpolation over gaps in ADS-B.
- PSFlights is used as the preferred aircraft performance model, with a fallback to BADA3 (aircraft type in the performance model being determined from ADS-B).
- Engine type is assigned to an aircraft based on ch-aviation's aircraft ICAO address (or tail number) to engine type mappings. If an aircraft is not present in the ch-aviation dataset, a default engine type is assigned based on  [this publication](https://acp.copernicus.org/articles/24/725/2024/acp-24-725-2024-supplement.pdf).
- The pycontrails version, aircraft performance model, and nvPM dataset used in computing individual flights are available in the CSV export on the Explorer tab.
- Our estimates will differ from those used in NEATS for the EU MRV, as NEATS uses different inputs and modeling parameters. 

### Flights Included
- The Contrails.org Impact Inventory excludes certain flights, for reasons including but not limited to:
  * ADS-B telemetries deemed to be erroneous by our QA/QC flight-validation process. The QA/QC flight-validation process first attempts to heal poor ADS-B data by selectively pruning spurious data in otherwise well-formed flight trajectories. If the trajectory is still presumed erroneous after a healing attempt, it is ejected.
  * Aircraft type not recognized by either aircraft performance model (mainly propeller or some private aircraft).
  * Aircraft does not fly higher than 22,664 ft at any point in its trajectory.
...
graph 
    ...
    bq_sync_util(BQ -> PSDB sync util)

    contrails_api(Contrails API)
    
    subgraph hyperdisk_1[Disk: HyperdiskML]
        met_cache(hyperdiskml: era5/hres zarr cache)
    end
    style hyperdisk_1 fill:#926de8
    subgraph gcs_1[Cloud Storage]
        spire_gcs_cache(bucket: contrails-301217-spire-cache-prod)
    end
    style gcs_1 fill:#03cffc
    subgraph gcs_2[Cloud Storage]
        met_gcs_cache(bucket: era5/hres zarr stores)
    end
    style gcs_2 fill:#03cffc
    ...
    subgraph k8s_3[Kubernetes]
        trajectory_worker(dep: trajectory-worker)
    end
    ...
    subgraph k8s_7[Kubernetes]
        psdb_proxy_dep(dep: psdb-proxy)
        psdb_proxy_svc(svc: psdb--proxy)
    end
    ...
    spire_ingest_raw_bq_queue(spire-ingest-raw-bq-queue)        
    end
    style pub_sub2 fill:#0864C8
    subgraph pub_sub4[PubSub]
        twjd_queue(twjd-queue)
    end
    style pub_sub4 fill:#0864C8
    subgraph pub_sub6[PubSub]
        traj_worker_queue(traj-worker-queue)
    end
    style pub_sub6 fill:#0864C8
    subgraph pub_sub7[PubSub]
        traj_worker_cocip_bq_queue(traj-worker-cocip-bq-queue)
    end
    ...
    style cli fill:#c4c708
    subgraph sql_1[Cloud SQL]
        psdb_inventory(psdb: impact-inventory)
    end
    ...
    spire_ingest_api_scraper --> spire_ingest_raw_bq_queue    
    spire_ingest_raw_bq_queue --> spire_flights_raw_tb
    
    traj_worker_queue --> trajectory_worker
    trajectory_worker --> traj_worker_cocip_bq_queue
    traj_worker_cocip_bq_queue --> trajectory_cocip_tb
    
    trajectory_cocip_tb --> bq_sync_util
    bq_sync_util --> psdb_inventory

    twjd_queue --> twjf
    spire_gcs_cache --> twjf
    spire_flights_raw_tb --> twjf
    twjf <--> twjf_cache
    twjf --> traj_worker_queue
    met_cache <--> twjf
    met_gcs_cache <--> twjf

    jobworker_submit --> twjd_queue
