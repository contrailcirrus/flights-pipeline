...
RUN_URI_GLOB="gs://contrails-301217-flights-pipeline-prod/logs/inventory_2023_run_jun2026/twjf-logs/"
...
TARGET_TABLE="logs_inventory_2023_run_june2026"

# For each file in RUN_URI_GLOB, load data  into BQ table 
gsutil ls -r $RUN_URI_GLOB | grep .json | tr '\n' '\0' | xargs -0 -n1 bq load --schema_update_option=ALLOW_FIELD_ADDITION --ignore_unknown_values --source_format=NEWLINE_DELIMITED_JSON --max_bad_records=40 --schema=./logs_bq_table_schema.json flights_pipeline_prod.${TARGET_TABLE}