...
WHERE jsonPayload.message LIKE "%skipping%"
    QUALIFY ROW_NUMBER() OVER (PARTITION BY jsonPayload.flight_id) = 1)
    , final_time_tb AS (
SELECT SUM (TIMESTAMP_DIFF(time_end, time_start, MINUTE)) AS total_final_flight_time_mins, TIMESTAMP_TRUNC(time_start, MONTH) AS flight_month_bin
FROM logs_tb
...
GROUP BY flight_month_bin),
    summary_tb
    AS (
SELECT COALESCE (bin_success_tb.flight_month_bin, bin_skipped_tb.flight_month_bin) AS flight_month_bin, COALESCE (bin_success_tb.total_flight_time_mins, 0) AS total_passed_flight_time_minutes, COALESCE (bin_skipped_tb.total_flight_time_mins, 0) AS total_skipped_flight_time_minutes, COALESCE (final_time_tb.total_final_flight_time_mins, 0) AS total_final_flight_time_minutes
...
SELECT *,
       ...
       100                                                                    AS skipped_perc,
       (total_skipped_flight_time_minutes + total_passed_flight_time_minutes - total_final_flight_time_minutes) * 100 /
       (total_skipped_flight_time_minutes + total_passed_flight_time_minutes) AS total_dropped_perc
FROM summary_tb
