...
resource "google_project_iam_member" "default_monitoring_viewer_binding" {
  member  = data.google_compute_default_service_account.default.member
  project = "contrails-301217"
  role    = "roles/monitoring.viewer" # Required for GKE custom metrics adapter
}

resource "google_service_account_iam_binding" "default_iam_workload_identity_user_binding" {
  service_account_id = data.google_compute_default_service_account.default.name
  role               = "roles/iam.workloadIdentityUser" # Required for GKE custom metrics adapter
  members = [
    # See: https://github.com/GoogleCloudPlatform/k8s-stackdriver/blob/master/custom-metrics-stackdriver-adapter
    "serviceAccount:contrails-301217.svc.id.goog[custom-metrics/custom-metrics-stackdriver-adapter]",
  ]
}
