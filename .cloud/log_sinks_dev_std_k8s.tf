...
resource "google_logging_project_sink" "tw_logs_sink_dev_std" {
  name        = "trajectory-worker-gaia-dev-std-skipped-logs"
  ...
  filter      = "resource.type=\"k8s_container\" AND resource.labels.project_id=\"contrails-301217\" AND resource.labels.location=\"us-east4\" AND resource.labels.cluster_name=\"contrails-gke-general-std-useast4\" AND resource.labels.namespace_name=\"flights-pipeline-dev\" AND labels.k8s-pod/app=\"trajectory-worker-gaia\" AND severity>=INFO"

  disabled = false
  unique_writer_identity = true
}

resource "google_logging_project_sink" "tw_backup_logs_sink_dev_std" {
  name        = "trajectory-worker-gaia-dev-std-backup-logs"
  ...
  filter      = "resource.type=\"k8s_container\" AND resource.labels.project_id=\"contrails-301217\" AND resource.labels.location=\"us-east4\" AND resource.labels.cluster_name=\"contrails-gke-general-std-useast4\" AND resource.labels.namespace_name=\"flights-pipeline-dev\" AND labels.k8s-pod/app=\"trajectory-worker-gaia-backup\""
