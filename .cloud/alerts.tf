...
resource "google_monitoring_alert_policy" "pubsubsubscription_prod_trajectory_worker_gaia_ingress_ack_count" {
  display_name = "pubsubsubscription-${google_pubsub_subscription.prod_trajectory_worker_gaia_chunk_ingress.name}-ack-count"
  combiner     = "OR"
  ...
  conditions {
    ...
    condition_monitoring_query_language {
      query    = <<EOF
        fetch pubsub_subscription
        | metric 'pubsub.googleapis.com/subscription/ack_message_count'
        | filter resource.subscription_id == '${google_pubsub_subscription.prod_trajectory_worker_gaia_chunk_ingress.name}'
        | group_by sliding(30m), aggregate(value.ack_message_count)
        | every 1m
        | condition val() < 10
        EOF
...
resource "google_monitoring_alert_policy" "pubsubsubscription_prod_trajectory_worker_gaia_ingress_oldest_message" {
  display_name = "pubsubsubscription-${google_pubsub_subscription.prod_trajectory_worker_gaia_chunk_ingress.name}-oldest-message"
  combiner     = "OR"
