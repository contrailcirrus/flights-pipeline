...
resource "google_monitoring_alert_policy" "pubsubsubscription_prod_trajectory_worker_realtime_chunk_ingress_ack_count" {
  display_name = "pubsubsubscription-${google_pubsub_subscription.prod_trajectory_worker_realtime_chunk_ingress.name}-ack-count"
  combiner     = "OR"
  ...
  conditions {
    ...
    condition_monitoring_query_language {
      query    = <<EOF
        fetch pubsub_subscription
        | metric 'pubsub.googleapis.com/subscription/ack_message_count'
        | filter resource.subscription_id == '${google_pubsub_subscription.prod_trajectory_worker_realtime_chunk_ingress.name}'
        | group_by sliding(30m), aggregate(value.ack_message_count)
        | every 1m
        | condition val() < 10
        EOF
...
resource "google_monitoring_alert_policy" "pubsubsubscription_prod_trajectory_worker_realtime_chunk_ingress_oldest_message" {
  display_name = "pubsubsubscription-${google_pubsub_subscription.prod_trajectory_worker_realtime_chunk_ingress.name}-oldest-message"
  combiner     = "OR"
...
resource "google_monitoring_alert_policy" "pubsubtopic_prod_trajectory_worker_cocip_egress_bigquery_publish_count" {
  display_name = "pubsubtopic-${google_pubsub_topic.prod_trajectory_worker_cocip_egress_bigquery.name}-publish-count"
  combiner     = "OR"
  ...
  conditions {
    ...
    condition_monitoring_query_language {
      query    = <<EOF
        fetch pubsub_topic
        | metric 'pubsub.googleapis.com/topic/message_sizes'
        | filter resource.topic_id == '${google_pubsub_topic.prod_trajectory_worker_cocip_egress_bigquery.name}'
        | group_by sliding(30m), row_count()
        | every 1m
        | condition val() < 10
        EOF
