...
resource "google_monitoring_alert_policy" "k8sdeployment_trajectory_worker_prod_error_in_logs" {
  display_name = "k8sdeployment-trajectory-worker-prod-error-in-logs"
  combiner     = "OR"

  conditions {
    display_name = "Error in logs"
    # Matches both trajectory-worker-realtime and trajectory-worker-batch
    condition_matched_log {
      filter = <<EOF
        resource.type="k8s_container"
        resource.labels.cluster_name="contrails-gke-general"
        resource.labels.namespace_name="flights-pipeline-prod"
        labels.k8s-pod/app:"trajectory-worker-"
        severity>=ERROR
        EOF
    }
  }
