...
def test_airports_csv_has_eu_data():
    ...
    for col in required_columns:
        assert col in airports_df.columns, f"Required column '{col}' not found in airports.csv"
    
    eu_airports_data = airports_df[airports_df['continent'] == 'EU']
    # Current count: 1447 EU airports + 10 buffer for data variations
    assert len(eu_airports_data) >= 1437, f"Expected at least 1437 EU airports, found {len(eu_airports_data)}"
...
def test_eu_airport_identification():
    ...
    for airport in known_eu_airports:
        assert eu_airports.is_eu_airport(airport), f"Airport {airport} should be identified as EU"
    
    # Test known non-EU airports
    known_non_eu_airports = ["KJFK", "KLAX", "RJAA", "ZBAA"]
    for airport in known_non_eu_airports:
        assert not eu_airports.is_eu_airport(airport), f"Airport {airport} should not be identified as EU" 