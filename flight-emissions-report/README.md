...
The CLI is invoked by running `./cli.py` (it is _not_, by design, an installable CLI).
...
and submits trajectory worker job descriptors (TWJDs) to the trajectory worker job factory.
...
### Manual Job Dispatch (`jobworker submit` )
This method sends a trajectory worker job descriptor (TWJD) to the trajectory worker job factory.
The trajectory worker job factory consumes the TWJD, and mints one or more jobs for the trajectory
worker to process.
