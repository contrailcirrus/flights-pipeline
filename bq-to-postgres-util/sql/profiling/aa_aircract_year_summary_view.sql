WITH airline_aggs AS (
    SELECT
        v.airline_iata,
        SUM(v.flight_cnt) AS flight_cnt,
        SUM(v.total_ef_mj) AS total_ef_mj,
        SUM(v.total_len_km) AS total_len_km,
        (SUM(v.total_ef_mj) / NULLIF(SUM(v.total_len_km), 0)) AS avg_ef_mj_per_km
    FROM inventory_monthly_airlines_stats v
    WHERE
        v.month_bucket >= '2024-01-01' AND v.month_bucket <= '2024-12-31'
