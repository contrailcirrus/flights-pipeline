...
from lib import environment, handlers, utils
...
def run(
    ...
    if cached:
        ...
        logger.info(f"icao_address: {job.flight_info.icao_address}. " f"cache miss.")
    ...
    try:
        if validated_cache:
            logger.info(f"icao_address: {job.flight_info.icao_address}. cache valid.")
        else:
            logger.info(
    ...
    cache_ts = [
        int(datetime.fromisoformat(w.timestamp).timestamp()) for w in validated_cache
    ]
    ...
    # ===================
    if (
        validated_flight_info.aircraft_type_icao in PerfModelLookup().aircraft_type_icao
