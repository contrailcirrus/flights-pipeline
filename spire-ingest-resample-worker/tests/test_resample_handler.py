...
recs_multi_min_span = SpireWaypointsRecord.from_utf8_json(
    ingress_pubsub_bytes["multi_min_span"]
).records
recs_gt_one_min = SpireWaypointsRecord.from_utf8_json(
    ingress_pubsub_bytes["gt_one_min_span"]
).records
recs_lt_inter_one_min = SpireWaypointsRecord.from_utf8_json(
    ingress_pubsub_bytes["lt_inter_one_min_span"]
).records
recs_lt_intra_one_min = SpireWaypointsRecord.from_utf8_json(
    ingress_pubsub_bytes["lt_intra_one_min_span"]
).records


def test_interpolate_points_within_single_minute_returns_no_waypoints():
    """
    ...
    h = lib.handlers.ResampleHandler(
        cache=[],
        records_window=recs_lt_intra_one_min,
    )
    interpolated_waypoints = h.interpolate().waypoints_resampled
    assert len(interpolated_waypoints) == 0


def test_interpolate_points_lt_one_min_spanning_next_minute_returns_one_waypoint():
    """
    case: no cache, multiple records,
    ...
    h = lib.handlers.ResampleHandler(
        cache=[],
        records_window=recs_lt_inter_one_min,
    )
    interpolated_waypoints = h.interpolate().waypoints_resampled
    assert len(interpolated_waypoints) == 1
